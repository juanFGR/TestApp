...
appControllers.controller('menuCtrl', function ($rootScope, $scope, $timeout, $mdUtil, $mdSidenav, $log, $ionicHistory, $state, $ionicPlatform, $mdDialog, $mdBottomSheet, $mdMenu, $mdSelect,svmenu) {
    ...
    if(Parse.User.current()){
              svmenu.load().then(function(data) {
          $scope.user = {
...
$scope.uploadProfileImage = function() {
  ...
  navigator.camera.getPicture(function(imageURI){
    ...
    parseFile.save().then(function() {
        ...
        //$("#userProfile_image").attr("src", arguments[0]._url);
        uri = arguments[0]._url;
        $rootScope.auth.image = uri;
         $rootScope.$apply();
        var query = new Parse.Query(Parse.Object.extend("Profile"));
        query.equalTo("objectId", Parse.User.current().get("profile").id);
        query.first({
            success: function(results) {
                results.set("image", uri);
                results.save();
                sessionStorage.setItem('profile', JSON.stringify(results));
            },
                ...
                if(jQuery('[id^=dialog]').length == 0 ) {
                    ...
                    $mdDialog.show({
                        ...
                        locals: {
                            displayOption: {
                                title: "Confirmación",
                                content: "Quieres salir de DOCARDY?",
                                ok: "Si",
                                cancel: "No"
                            }
                        }
