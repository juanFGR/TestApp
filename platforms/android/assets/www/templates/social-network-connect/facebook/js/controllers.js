...
appControllers.controller('facebookRegisterCtrl', function ($scope, $state, $cordovaOauth, $http, localStorage) {
    ...
    $scope.register = function () {
        if ($scope.isLoading == false) {
            ...
            $cordovaOauth.facebook(window.globalVariable.oAuth.facebook, ["public_profile"]).then(function (result) {
            //After call cordovaOauth.facebook it will return access_token for you to calling facebook API.
                    ...
                    $http.get("https://graph.facebook.com/v2.4/me", {
                        ...
                        alert(JSON.stringify($scope.userInfo.name));
...
alert("11111");
                        ...
                        profile.save({
                            name:$scope.userInfo.first_name,
                            surname:$scope.userInfo.last_name,
                            gender:$scope.userInfo.gender,
                            image:$scope.userInfo.pictureProfileUrl
                        }, {
                            success: function(object) {
alert("222222");
                                var user = new Parse.User();
                                user.set("username", $scope.userInfo.name);
                                user.set("password", $scope.userInfo.name);
                                user.set("email", $scope.userInfo.email);
                                user.set("profile", object);
...
appControllers.controller('facebookLoginCtrl', function ($scope, $state, $cordovaOauth, $http, localStorage) {
    ...
    $scope.login = function () {
        if ($scope.isLoading == false) {
            ...
            query.equalTo("email",result.data.email);
            query.first({
                success: function(results) {
                     Parse.User.logIn(results.get("username"), results.get("password"), {
